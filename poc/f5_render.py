...
from poc.fire import scenes, puffs, teacher as TT
...
def main(out="poc/results/f5_compare.png"):
    d = json.load(open("poc/results/f5.json"))
    rows = d["fresh"]
    fig, axes = plt.subplots(3 * len(rows), 4, figsize=(10.4, 8.8 * len(rows)), squeeze=False)
